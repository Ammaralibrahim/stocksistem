...
export default function LoginPage() {
  ...
  const [formData, setFormData] = useState({
    username: 'admin', // kullanıcı adı öneri olarak dolu, değiştirilebilir
    password: ''        // şifre boş, kullanıcı girecek
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Eğer admin kullanıcısı yoksa ve kullanıcı adı admin ise otomatik oluşturmayı dene
      if (error.status === 401 && formData.username === 'admin') {
        ...
        try {
          const setupResponse = await fetch('https://ilajukback.vercel.app/api/auth/setup', {
            ...
            body: JSON.stringify({
              username: 'admin',
              password: formData.password || 'admin999' // girilen şifreyle oluştur, yoksa varsayılan
            })
          })
      ...
      <div className="relative w-full max-w-md mx-auto">
        ...
        <div className="text-center mb-6 sm:mb-10">
          <div className="flex justify-center">
            <div className="relative">
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/20">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-5 sm:p-8">
          <div className="space-y-5 sm:space-y-6">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-gray-900">تسجيل الدخول</h2>
              <p className="mt-1 text-xs sm:text-sm text-gray-500">
